...
Diagnosis_age = sample(1:3, cohortSize, c(7.5, 35,57.5), replace = T)
...
for(i in 1:cohortSize)
{
  ...
  #gf <- 3 * exp(-0.00946 * 1:98)
  w1 = rexp(1, rate = 1.5)

    inf_year =  mysamp(n=1, m=1992, s=14.17, lwr=1980, upr=2018, nnorm=1000)+w1

  age_inf = mysamp(n=1, m=19.4, s=13.7, lwr=14, upr=40, nnorm=1000) + w1

  bl[i,"BirthYear"] = inf_year - age_inf
  bl[i,"Age"] = age_inf
  
  # Diagnosis_age = sample(1:4, cohortSize, c(0.645,8.53,76.21,13.61), replace = T)
  # > for(i in 1:cohortSize)
  #   +  {age_ID = Diagnosis_age[i]
  #   +  age = switch(age_ID, runif(1,5,14),runif(1,14,19), runif(1,20,49),runif(1,50,75))
  #   +  }
  # > for(i in 1:cohortSize)
  #   +  {age_ID = Diagnosis_age[i]
  #   +  age = switch(age_ID, runif(1,5,14),runif(1,14,19), runif(1,20,49),runif(1,50,75))
  #   +  bl[i,"Age"] = age}
  # > hist(bl[,"Age"])
  # > hist(bl[,"Age"],breaks=10, col="grey", xlab="Age at infection", 
  #        +      main="Distribution of age at HCV infection among non-IDUs population")
...
bl[,"Gender"] <- rbinom(cohortSize, 1, 0.9920)
bl[,"HIV"] <- rbinom(cohortSize, 1, 0.023)
bl[,"IDU"] <- rbinom(cohortSize, 1, 1)
####################################################################
bl[,"HIV_time"] <- (bl[,"HIV"])
#######################
for (i in 1: cohortSize)
